...
(
    ...
    input  decode_exception_t          fetch_exception,
    ...
    // Only an actual control-flow/exception class discards the sequential
    // packet mate. Serializing non-control operations (LL/SC and barriers)
    // keep that instruction in the queue and merely prevent dual issue.
    wire slot0_static_kill = slot0_effective_dec.is_jump;
    ...
    always_comb begin
        ...
        kill_after_slot0 =
            (fetch_exception != EXCEPTION_NONE)
            || slot0_static_kill
            || slot0_pred_taken;
        ...
        entry0.fetch_exception = fetch_exception;
        if (fetch_exception != EXCEPTION_NONE) begin
            entry0.inst = 32'd0;
            entry0.is_conditional_branch = 1'b0;
            entry0.is_direct_jump = 1'b0;
            entry0.is_indirect_jump = 1'b0;
            entry0.is_privileged = 1'b1;
            entry0.is_privileged_flow = 1'b1;
            entry0.is_illegal = 1'b0;
            entry0.is_muldiv = 1'b0;
            entry0.is_mul = 1'b0;
            entry0.is_load = 1'b0;
            entry0.is_store = 1'b0;
            entry0.is_alu_type = 1'b0;
            entry0.writes_dst = 1'b0;
            entry0.uses_src0 = 1'b0;
            entry0.uses_src1 = 1'b0;
            entry0.is_jump = 1'b1;
            entry0.is_control = 1'b1;
            entry0.is_lsu = 1'b0;
            entry0.force_single = 1'b1;
        end
        entry1 = make_entry(
            enq1_valid,
            slot1_pc,
        ...
        entry1.fetch_exception = EXCEPTION_NONE;
