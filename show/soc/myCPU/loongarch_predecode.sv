...
(
    ...
    wire inst_dbar    = op17 == LA_OP_DBAR;
    wire inst_ibar    = op17 == LA_OP_IBAR;
    wire inst_idle    = op17 == LA_OP_IDLE;
    wire inst_cacop   = op10 == {6'h01, 4'h8};
    wire is_barrier   = inst_dbar | inst_ibar;
    ...
    wire inst_preld = op10 == LA_OP_PRELD;
    wire inst_ll_w   = inst[31:24] == LA_OP_LL_W;
    wire inst_sc_w   = inst[31:24] == LA_OP_SC_W;
    ...
    wire inst_tlbsrch = inst == 32'h0648_2800;
    wire inst_tlbrd   = inst == 32'h0648_2c00;
    wire inst_tlbwr   = inst == 32'h0648_3000;
    wire inst_tlbfill = inst == 32'h0648_3400;
    wire inst_ertn    = inst == 32'h0648_3800;
    wire inst_invtlb_encoding = op17 == 17'h0c93;
    wire inst_invtlb = inst_invtlb_encoding && (inst[4:0] <= 5'd6);
    wire inst_tlb = inst_tlbsrch | inst_tlbrd | inst_tlbwr | inst_tlbfill
                  | inst_invtlb;
    ...
    wire is_privileged = inst_csr | inst_syscall | inst_ertn | inst_break
                       | inst_counter | inst_cpucfg | inst_tlb | inst_cacop
                       | inst_idle;
    ...
    wire is_load = inst_ld_b | inst_ld_h | inst_ld_w
                 | inst_ld_bu | inst_ld_hu | inst_ll_w | inst_preld;
    wire is_store = inst_st_b | inst_st_h | inst_st_w | inst_sc_w;
    wire is_atomic = inst_ll_w | inst_sc_w | inst_preld;
    ...
    wire instruction_legal = is_alu_rr | is_alu_imm | is_upper_imm
                           ...
                           | is_privileged | inst_break | is_barrier;
    ...
    wire slot1_allowed = is_alu_rr | is_alu_imm | is_upper_imm
                       | (is_load & ~is_atomic)
                       | (is_store & ~is_atomic) | is_conditional
                       | is_direct | inst_jirl;
    wire younger_allowed = is_alu_rr | is_alu_imm | is_upper_imm
                         | is_mul | (is_load & ~is_atomic)
                         | (is_store & ~is_atomic)
    ...
    always_comb begin
        ...
        decoded.writes_dst = is_alu_rr | is_alu_imm | is_upper_imm
                           | is_muldiv | (is_load & ~inst_preld) | inst_sc_w
                           | inst_bl | inst_jirl
...
(
    ...
    wire inst_counter = ((op17 == 17'd0)
                      ...
                      || ((op17 == 17'd0)
                          ...
                          && (inst[9:5] == 5'd0));
    wire inst_cpucfg = (op17 == 17'd0) && (inst[14:10] == 5'd27);
    wire inst_tlb = (inst == 32'h0648_2800)
                  | (inst == 32'h0648_2c00)
                  | (inst == 32'h0648_3000)
                  | (inst == 32'h0648_3400)
                  | ((op17 == 17'h0c93) && (inst[4:0] <= 5'd6));
    wire inst_ll_w = inst[31:24] == LA_OP_LL_W;
    wire inst_sc_w = inst[31:24] == LA_OP_SC_W;
    wire inst_preld = inst[31:22] == LA_OP_PRELD;
    wire inst_idle = op17 == LA_OP_IDLE;
    wire inst_barrier = (op17 == LA_OP_DBAR) | (op17 == LA_OP_IBAR)
                      ;
    wire inst_cacop = inst[31:22] == {6'h01, 4'h8};
    ...
    always_comb begin
        ...
        if (decoded.is_alu_type && decoded.uses_src0 && decoded.uses_src1)
            ...
            metadata.inst_kind = ICACHE_KIND_LL;
        else if (inst_sc_w)
            metadata.inst_kind = ICACHE_KIND_SC;
        else if (inst_preld)
            metadata.inst_kind = ICACHE_KIND_PRELD;
        else if (inst_barrier)
            metadata.inst_kind = ICACHE_KIND_BARRIER;
            ...
            metadata.inst_kind = ICACHE_KIND_TLB;
        else if (inst_cacop)
            metadata.inst_kind = ICACHE_KIND_CACOP;
        else if (inst_idle)
            metadata.inst_kind = ICACHE_KIND_IDLE;
...
(
    ...
    always_comb begin
        ...
        kind_ll = cached.inst_kind == ICACHE_KIND_LL;
        kind_sc = cached.inst_kind == ICACHE_KIND_SC;
        kind_barrier = cached.inst_kind == ICACHE_KIND_BARRIER;
        ...
        kind_tlb = cached.inst_kind == ICACHE_KIND_TLB;
        kind_cacop = cached.inst_kind == ICACHE_KIND_CACOP;
        kind_preld = cached.inst_kind == ICACHE_KIND_PRELD;
        kind_idle = cached.inst_kind == ICACHE_KIND_IDLE;
        kind_priv_flow = cached.inst_kind == ICACHE_KIND_PRIV_FLOW;
        kind_illegal = cached.inst_kind == ICACHE_KIND_ILLEGAL;
        inst_invtlb = (inst[31:15] == 17'h0c93) && (inst[4:0] <= 5'd6);
        ...
        expanded.is_privileged = kind_csr | kind_counter | kind_counter_id
                               | kind_cpucfg | kind_tlb | kind_cacop
                               | kind_idle | kind_priv_flow;
        ...
        expanded.is_load = kind_load | kind_ll | kind_preld;
        expanded.is_store = kind_store | kind_sc;
        ...
        expanded.uses_src0 = kind_alu_rr | kind_alu_imm | kind_muldiv
                           | kind_load | kind_store | kind_ll | kind_sc
                           | kind_conditional
                           | kind_jirl | kind_csr_write
                           | kind_csr_exchange | kind_cpucfg | inst_invtlb
                           | kind_cacop | kind_preld;
        expanded.uses_src1 = kind_alu_rr | kind_muldiv | kind_store | kind_sc
                           | kind_conditional | kind_csr_exchange
                           | inst_invtlb;
        expanded.src0_addr = kind_csr ? inst[4:0] : inst[9:5];
        expanded.src1_addr = kind_csr ? inst[9:5]
                           : (kind_store | kind_sc | kind_conditional)
                               ? inst[4:0] : inst[14:10];
        ...
        expanded.is_control = kind_conditional | expanded.is_jump;
        expanded.is_lsu = kind_load | kind_store | kind_ll | kind_sc
                        | kind_preld;
