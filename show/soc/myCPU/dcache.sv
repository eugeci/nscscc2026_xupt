...
module dcache #(
    // These parameters provide the direct-address fallback cacheability
    // window.  Paging may mark any physical page cacheable through MAT, so the
    // cache itself always retains a complete physical tag.
    ...
    // CACOP maintenance request. mode 0 discards the indexed way, mode 1
    // writes back a dirty indexed way then invalidates it, and mode 2 does
    // the same only when the physical tag hits.
    input  logic        maint_valid,
    input  logic [31:0] maint_addr,
    ...
    localparam TAG_W      = 18;   // physical addr[31:14]
    ...
    // ================================================================
    wire [TAG_W-1:0]   ex_tag   = cpu_addr[31:14];
    ...
    typedef enum logic [3:0] {
        ...
        S_MAINT_WB_WAIT,  // maintenance waits for dirty-line B success
        S_MAINT_DONE      // one-cycle completion acknowledgement
    } state_t;
    ...
    wire state_maint_done    = (state == S_MAINT_DONE);
    wire refill_start;
    ...
    // Keep all set-valid bits for one way in a packed vector. Besides mapping
    // naturally to resettable flops, this avoids simulator-specific handling
    // of nonblocking writes to a doubly-unpacked array during reset.
    logic [SETS-1:0] tag_vld [WAYS-1:0];
    ...
    // Maintenance performs an asynchronous metadata lookup while the normal
    // CPU pipeline is quiescent. Index-class operations select the way with
    // address bit zero, matching OpenLA500's CACOP address convention.
    logic maint_active_q;
    wire [INDEX_W-1:0] maint_index = maint_addr[13:5];
    wire [TAG_W-1:0] maint_tag = maint_addr[31:14];
    wire [TAG_BANK_INDEX_W-1:0] maint_tag_set =
        maint_index[TAG_BANK_INDEX_W-1:0];
    wire [TAG_W-1:0] maint_tag_way0 = maint_index[INDEX_W-1]
        ? tag_mem_way0_bank1[maint_tag_set]
        : tag_mem_way0_bank0[maint_tag_set];
    wire [TAG_W-1:0] maint_tag_way1 = maint_index[INDEX_W-1]
        ? tag_mem_way1_bank1[maint_tag_set]
        : tag_mem_way1_bank0[maint_tag_set];
    wire maint_hit_way0 = tag_vld[0][maint_index]
                         & (maint_tag_way0 == maint_tag);
    wire maint_hit_way1 = tag_vld[1][maint_index]
                         & (maint_tag_way1 == maint_tag);
    wire maint_hit = maint_hit_way0 | maint_hit_way1;
    wire maint_selected_way = (maint_mode == 2'd2)
                            ? maint_hit_way1 : maint_addr[0];
    wire maint_selected_valid = (maint_mode == 2'd2)
                              ? maint_hit
                              : tag_vld[maint_selected_way][maint_index];
    wire [TAG_W-1:0] maint_selected_tag = maint_selected_way
                                               ? maint_tag_way1
                                               : maint_tag_way0;
    wire maint_selected_dirty = maint_selected_way
                              ? dirty_way1[maint_index]
                              : dirty_way0[maint_index];
    wire maint_needs_writeback = (maint_mode != 2'd0)
                               & maint_selected_valid
                               & maint_selected_dirty;
    wire maint_start = maint_valid & state_idle & ~maint_active_q
                     & ~mem_req & (wb_state == WB_IDLE);
    wire maintenance_owner_start = refill_start | maint_start;
    wire maint_invalidate_now = (maint_start & ~maint_needs_writeback
                                 & maint_selected_valid)
                              | (wb_resp_ok & maint_active_q);
    assign maint_done = state_maint_done;
    ...
    // Compare six physical-tag bits per group in parallel before the EX->MEM
    // edge.  Complete PA tags allow MAT to cache arbitrary Linux physical
    // pages without aliases between regions that share addr[18:0].
    ...
    wire [2:0] tag_eq_group_w0_b0 = {
        ~|tag_diff_w0_b0[17:12], ~|tag_diff_w0_b0[11:6],
        ~|tag_diff_w0_b0[5:0]
    };
    wire [2:0] tag_eq_group_w0_b1 = {
        ~|tag_diff_w0_b1[17:12], ~|tag_diff_w0_b1[11:6],
        ~|tag_diff_w0_b1[5:0]
    };
    wire [2:0] tag_eq_group_w1_b0 = {
        ~|tag_diff_w1_b0[17:12], ~|tag_diff_w1_b0[11:6],
        ~|tag_diff_w1_b0[5:0]
    };
    wire [2:0] tag_eq_group_w1_b1 = {
        ~|tag_diff_w1_b1[17:12], ~|tag_diff_w1_b1[11:6],
        ~|tag_diff_w1_b1[5:0]
    };
    ...
    logic [2:0] mem_tag_eq_w0_b0;
    logic [2:0] mem_tag_eq_w0_b1;
    logic [2:0] mem_tag_eq_w1_b0;
    logic [2:0] mem_tag_eq_w1_b1;
...
`endif
    ...
    // mem_uncached is authoritative and may come from either the direct-mode
    // platform window or MMU MAT.  Full tags make both classifications safe.
    ...
    wire wb_resp_fire = wb_state_resp & backend_wr_valid
                      & backend_wr_ready;
    assign wb_resp_ok = wb_resp_fire & (mem_wr_resp == 2'b00);
        ...
        case (state)
            S_IDLE: begin
                if (maint_start) begin
                    if (maint_needs_writeback)
                        state_next = S_WB_CAPTURE;
                    else
                        state_next = S_MAINT_DONE;
                end else if (idle_uncached_start)
                    state_next = S_UC_REQ;
            ...
            S_REPLAY:
                state_next = S_IDLE;
            S_WB_CAPTURE: begin
                if (~maint_active_q & refill_abort)
            ...
            S_WB_REQ: begin
                if (wb_req_fire)
                    ...
                    state_next = maint_active_q ? S_MAINT_WB_WAIT
                                                : S_REFILL_REQ;
                else if (~maint_active_q & refill_abort)
                    state_next = S_IDLE;
            end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            wb_state <= WB_IDLE;
        else if (maintenance_owner_start)
            wb_state <= WB_IDLE;
        else begin
            case (wb_state)
                WB_IDLE: begin
                    if (wb_capture_last
                        & (maint_active_q | ~refill_abort))
                        wb_state <= WB_CMD;
                end
                WB_CMD: begin
                    if (wb_req_fire)
                        wb_state <= WB_DATA;
                    else if (state_wb_req & ~maint_active_q & refill_abort)
                        wb_state <= WB_IDLE;
                end
                WB_DATA: begin
    ...
    always_ff @(posedge clk) begin
        ...
        end else if (maintenance_owner_start) begin
            wb_required <= maint_start ? maint_needs_writeback
                                       : victim_needs_writeback;
            wb_done <= 1'b0;
            refill_read_accepted <= 1'b0;
        end else begin
            if (wb_resp_ok)
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            refill_target_valid <= 1'b0;
            refill_target_data  <= 32'd0;
        end else if (maint_start) begin
            refill_way          <= maint_selected_way;
            refill_index        <= maint_index;
            victim_line_addr    <= {
                maint_selected_tag, maint_index, 5'b00000
            };
        end else begin
    ...
    always_ff @(posedge clk) begin
        if (maintenance_owner_start) begin
...
`endif
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (maint_invalidate_now) begin
                if (maint_start)
                    tag_vld[maint_selected_way][maint_index] <= 1'b0;
                else
                    tag_vld[refill_way][refill_index] <= 1'b0;
            end
    ...
    always_comb begin
        ...
        if (maint_invalidate_now) begin
            dirty_write_valid = 1'b1;
            dirty_write_way   = maint_start ? maint_selected_way
                                             : refill_way;
            dirty_write_index = maint_start ? maint_index : refill_index;
            dirty_write_data  = 1'b0;
        end
    end
...
`endif
    ...
    // Cached traffic and uncached writes retain the established aligned-word
    // transport.  An uncached read must preserve the architectural byte
    // address and access width: widening an IIR/status byte load into a word
    // can read a lower, side-effecting FIFO register before the requested
    // byte reaches the peripheral.
    assign mem_req_addr  = wb_state_cmd ? victim_line_addr
                         : state_uc_req
                             ? (mem_wr ? {mem_addr[31:2], 2'b00} : mem_addr)
                         : refill_fetch_addr;
    assign mem_req_size  = (state_uc_req & ~mem_wr)
                         ? {1'b0, mem_load_size} : 3'd2;
