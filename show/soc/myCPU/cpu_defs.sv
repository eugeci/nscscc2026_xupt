...
package cpu_defs;
    ...
    typedef enum logic [1:0] {
        ...
        ATOMIC_LL   = 2'b01,
        ATOMIC_SC   = 2'b10,
        // A prefetch reuses the load/cache-fill path without producing an
        // architectural load event or register result.
        ATOMIC_PREFETCH = 2'b11
    } atomic_op_t;
    ...
    typedef enum logic [3:0] {
        PRIV_NONE    = 4'b0000,
        PRIV_REG     = 4'b0001,
        PRIV_SYSCALL = 4'b0010,
        PRIV_RETURN  = 4'b0011,
        // LoongArch stable-counter reads share the privileged result path but
        // are not CSR accesses and remain legal outside PLV0.
        PRIV_COUNTER = 4'b0100,
        // CPUCFG also reuses the privileged result path, but is an
        // unprivileged, read-only architectural configuration query.
        PRIV_CPUCFG  = 4'b0101,
        // LA32R TLB maintenance operations are serialized in the same
        // privileged execution lane. PRIV_ADDR identifies the exact command.
        PRIV_TLB     = 4'b0110,
        // CACOP uses the privileged lane to preserve its five-bit cache code
        // and to share precise PLV/page-fault handling.  The cache transaction
        // itself is issued by cpu_top after older backend work has drained.
        PRIV_CACOP   = 4'b0111,
        // IDLE is privileged but, unlike a trap/return, normally completes
        // before the core enters its architectural wait state.
        PRIV_IDLE    = 4'b1000
    } priv_op_t;
    ...
    typedef enum logic [2:0] {
        EXCEPTION_NONE       = 3'b000,
        EXCEPTION_ILLEGAL    = 3'b001,
        EXCEPTION_BREAKPOINT = 3'b010,
        EXCEPTION_FETCH_TLBR = 3'b011,
        EXCEPTION_FETCH_PIF  = 3'b100,
        EXCEPTION_FETCH_PPI  = 3'b101
    } decode_exception_t;
    ...
    typedef struct packed {
        ...
        decode_exception_t fetch_exception;
    } fetch_slot_t;
    ...
    // sufficient for every LA32R family implemented by the NSCSCC core.
    // Static packet-mate suppression is derived from the expanded control-flow
    // class; odd values are not a semantic property because LL/SC and barriers
    // are serializing non-control operations.
    typedef enum logic [4:0] {
        ...
        ICACHE_KIND_LL           = 5'd11,
        ICACHE_KIND_DIVMOD       = 5'd12,
        ICACHE_KIND_SC           = 5'd13,
        ICACHE_KIND_CONDITIONAL  = 5'd14,
        ICACHE_KIND_BARRIER      = 5'd15,
        ...
        ICACHE_KIND_TLB          = 5'd28,
        ICACHE_KIND_PRELD        = 5'd29,
        ICACHE_KIND_CACOP        = 5'd30,
        ICACHE_KIND_IDLE         = 5'd31
    } icache_inst_kind_t;
