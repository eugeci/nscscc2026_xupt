...
//   - distributed full-physical-tag/class storage with one valid bit per line
...
module icache (
    ...
    input  logic        irom_req_cacheable,
    ...
    // CACOP maintenance request. mode 0/1 invalidate the indexed line;
    // mode 2 invalidates only on a physical-tag hit.
    ...
    // Paging can map executable pages anywhere in the 32-bit physical address
    // space.  Retain the complete physical tag above the 9-bit set index and
    // 4-bit line offset instead of relying on the boot-ROM 0x1c0 prefix.
    localparam integer TAG_WIDTH = 19;
    ...
    wire [TAG_WIDTH-1:0] irom_req_tag = irom_req_addr[31:13];
    wire irom_req_block = irom_req_addr[3];
    wire [INDEX_WIDTH:0] irom_req_data_row = {
        irom_req_index,
        irom_req_block
    };
    wire [INDEX_WIDTH-1:0] maint_index = maint_addr[12:4];
    wire [TAG_WIDTH-1:0] maint_tag = maint_addr[31:13];
    wire maint_hit = line_valid_q[maint_index]
                   & (tag_mem[maint_index][TAG_WIDTH-1:0] == maint_tag);
    wire maint_quiescent = (refill_state_q == REFILL_IDLE)
                          & ~lookup_valid_q
                          & ~pending_miss_valid_q
                          & ~miss_resp_valid_q;
    wire maint_start = maint_valid & ~maint_busy_q & maint_quiescent;
    wire maint_invalidate = maint_start
                          & ((maint_mode != 2'd2) | maint_hit);
    ...
    wire irom_req_tag_eq0 = ~|irom_req_tag_diff[4:0];
    wire irom_req_tag_eq1 = ~|irom_req_tag_diff[9:5];
    wire irom_req_tag_eq2 = ~|irom_req_tag_diff[14:10];
    wire irom_req_tag_eq3 = ~|irom_req_tag_diff[18:15];
    ...
    wire irom_req_refill_hit =
        ...
        & irom_req_refill_eq4;
    wire irom_req_hit = irom_req_cacheable
                      & (irom_req_array_hit | irom_req_refill_hit);
    ...
    always_ff @(posedge clk) begin
        if (irom_req_fire) begin
            ...
            lookup_refill_hit_q <= irom_req_cacheable
                                 & irom_req_refill_hit;
            lookup_cacheable_q <= irom_req_cacheable;
    ...
    // committed atomically with the physical line tag.
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            if (refill_line_start && refill_cacheable_q)
                line_valid_q[refill_index] <= 1'b0;
            if (maint_invalidate)
                line_valid_q[maint_index] <= 1'b0;
    ...
    always_ff @(posedge clk) begin
        ...
        if (lookup_miss & ~refill_matches_lookup)
            pending_miss_cacheable_q <= lookup_cacheable_q;
    end
    ...
    always_ff @(posedge clk) begin
        if (!rst_n)
            ...
            if (refill_state_q == REFILL_REQ)
                // A command whose VALID has reached the backend may not be
                // withdrawn while READY is low.  Keep presenting it until
                // the handshake, then drain the now-stale read response.
                refill_state_q <= mem_req_fire ? REFILL_DATA : REFILL_REQ;
        ...
        if (irom_req_kill) begin
            ...
            if (refill_state_q == REFILL_REQ) begin
                if (mem_req_fire) begin
                    ...
                    // Remember that the held command no longer owns a
                    // frontend request.  When it is eventually accepted its
                    // complete AXI response is consumed without allocation
                    // or a frontend response.
                    refill_drop_q <= 1'b1;
                end
            ...
            case (refill_state_q)
                REFILL_IDLE: begin
                    ...
                    if (pending_miss_valid_q) begin
                        ...
                        refill_cacheable_q <= pending_miss_cacheable_q;
                        refill_second_block_q <= 1'b0;
                ...
                REFILL_DATA: begin
                    if (mem_rd_fire) begin
                        ...
                        if (refill_drop_q) begin
                            ...
                            refill_beat_q <= 1'b0;
                            refill_block_resp_q <= 2'b00;
                            if (!refill_cacheable_q && mem_rd_last) begin
                                refill_second_block_q <= 1'b0;
                                refill_response_needed_q <= 1'b0;
    ...
    assign mem_req_len = refill_cacheable_q ? 8'd3 : 8'd1;
    assign mem_req_burst = refill_cacheable_q ? 2'b10 : 2'b01;
...
`ifndef SYNTHESIS
    always_ff @(posedge clk) begin
        ...
        if (rst_n
            ...
            && (mem_rd_last !=
                (refill_beat_q
                 & (!refill_cacheable_q || refill_second_block_q))))
            $error("ICache refill RLAST mismatch");
