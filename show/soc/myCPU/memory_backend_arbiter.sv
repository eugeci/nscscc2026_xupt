...
module memory_backend_arbiter #(
    ...
    logic command_grant_valid_q;
    logic command_grant_dcache_q;
    ...
    wire arbitrate_irom =
        ...
        & (~d_write_active_q | d_write_overlap_ok_q)
        & ~arbitrate_dcache;
    // Once a command is exposed with VALID high, its source must not change
    // until the downstream handshake.  Without this lock a later DCache
    // request (which has priority) can replace an already-stalled ICache
    // address and ID, violating AXI's payload-stability rule.
    wire select_dcache = command_grant_valid_q
                       ? command_grant_dcache_q : arbitrate_dcache;
    wire select_irom = command_grant_valid_q
                     ? ~command_grant_dcache_q : arbitrate_irom;
    ...
    assign m_req_valid = (select_dcache & d_req_valid)
                       | (select_irom & i_req_valid);
    assign m_req_write = select_dcache ? d_req_write : 1'b0;
    assign m_req_addr = select_dcache ? d_req_addr : i_req_addr;
    assign m_req_size = select_dcache ? d_req_size : 3'd2;
    ...
    always_ff @(posedge clk) begin
        if (!rst_n) begin
            ...
            command_grant_valid_q <= 1'b0;
            command_grant_dcache_q <= 1'b0;
        end else begin
            if (i_read_done)
                i_read_active_q <= 1'b0;
            if (d_read_done)
            ...
            end else if (!command_grant_valid_q && m_req_valid) begin
                command_grant_valid_q <= 1'b1;
                command_grant_dcache_q <= select_dcache;
            end
        end
    end
