...
(
    ...
    wire inst_invtlb = inst_invtlb_encoding && (rd <= 5'd6);
    ...
    wire is_load = inst_ld_b | inst_ld_h | inst_ld_w
                 | inst_ld_bu | inst_ld_hu | inst_ll_w;
    ...
    always_comb begin
        ...
        if (inst_cpucfg) begin
            ...
            uop.block_younger = 1'b1;
            uop.serializing = 1'b1;
        end else if (inst_tlb) begin
            uop.exec_unit = EXEC_PRIV;
            uop.src0_addr = rj;
            uop.src1_addr = rk;
            uop.src0_used = inst_invtlb;
            uop.src1_used = inst_invtlb;
            uop.priv_op = PRIV_TLB;
            uop.priv_addr = inst_tlbsrch ? 16'h0000
                         : inst_tlbrd   ? 16'h0001
                         : inst_tlbwr   ? 16'h0002
                         : inst_tlbfill ? 16'h0003
                                        : {8'h01, 3'd0, rd};
            uop.exception = EXCEPTION_NONE;
            uop.lane_mask = 2'b01;
            uop.block_younger = 1'b1;
            ...
            uop.block_younger = 1'b1;
            uop.serializing = 1'b1;
        end else if (inst_cacop) begin
            // CACOP code is carried in PRIV_ADDR while the ordinary ALU forms
            // VA=GR[rj]+SEXT(si12). Unsupported cache selectors/mode 3 remain
            // architecturally contained as serialized no-ops in cpu_top.
            uop.exec_unit = EXEC_PRIV;
            uop.src0_addr = rj;
            uop.src0_used = 1'b1;
            uop.operand_a_sel = OPERAND_A_SRC0;
            ...
            uop.priv_op = PRIV_CACOP;
            uop.priv_addr = {11'd0, rd};
            uop.exception = EXCEPTION_NONE;
            uop.lane_mask = 2'b01;
            uop.block_younger = 1'b1;
            uop.serializing = 1'b1;
        end else if (inst_preld) begin
            // A defined PRELD hint is executed through the ordinary cache
            // load/fill path, but never writes a GPR or exposes a load event.
            // PRIV_ADDR carries the five-bit hint to cpu_top.
            ...
            uop.block_younger = 1'b1;
            uop.serializing = 1'b1;
        end else if (inst_idle) begin
            uop.exec_unit = EXEC_PRIV;
            uop.priv_op = PRIV_IDLE;
            uop.priv_addr = {1'b0, inst[14:0]};
            uop.exception = EXCEPTION_NONE;
            uop.lane_mask = 2'b01;
            uop.block_younger = 1'b1;
            uop.serializing = 1'b1;
        end else if (inst_dbar | inst_ibar) begin
            // With a single in-order memory port, draining older backend
            // tokens and holding younger issue provides the architectural
            // DBAR/IBAR ordering point.
            ...
            uop.dst_write = 1'b1;
            uop.operand_b_sel = OPERAND_B_IMM;
            uop.imm = inst_ll_w ? la_imm_si14_shift2(inst)
                                : la_imm_si12(inst);
            uop.wb_src = WB_LOAD;
            uop.mem_cmd = MEM_LOAD;
            ...
            uop.atomic_op = inst_ll_w ? ATOMIC_LL : ATOMIC_NONE;
            uop.exception = EXCEPTION_NONE;
            uop.lane_mask = inst_ll_w ? 2'b01 : 2'b11;
            uop.block_younger = inst_ll_w;
            uop.serializing = inst_ll_w;
        end else if (is_store) begin
            uop.exec_unit = EXEC_LSU;
            uop.src0_used = 1'b1;
            uop.src1_used = 1'b1;
            uop.operand_b_sel = OPERAND_B_IMM;
            uop.imm = inst_sc_w ? la_imm_si14_shift2(inst)
                                : la_imm_si12(inst);
            uop.mem_cmd = MEM_STORE;
            uop.mem_size = inst_st_b ? MEM_BYTE
                          : inst_st_h ? MEM_HALF
                                      : MEM_WORD;
            uop.dst_write = inst_sc_w;
            uop.wb_src = inst_sc_w ? WB_EXEC : WB_NONE;
            uop.atomic_op = inst_sc_w ? ATOMIC_SC : ATOMIC_NONE;
            uop.exception = EXCEPTION_NONE;
            uop.lane_mask = inst_sc_w ? 2'b01 : 2'b11;
            uop.block_younger = inst_sc_w;
            uop.serializing = inst_sc_w;
