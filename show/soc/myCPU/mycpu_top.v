...
module core_top #(
    ...
    output wire         debug0_wb_mem_read,
    output wire         debug0_wb_mem_write,
    output wire  [31:0] debug0_wb_mem_addr,
    output wire  [31:0] debug0_wb_store_data,
    output wire         debug1_wb_mem_read,
    output wire         debug1_wb_mem_write,
    output wire  [31:0] debug1_wb_mem_addr,
    output wire  [31:0] debug1_wb_store_data,
    output wire  [ 3:0] debug1_wb_rf_wen_raw,
    output wire  [ 4:0] debug1_wb_rf_wnum_raw,
    output wire  [31:0] debug1_wb_rf_wdata_raw,
    output wire         debug_fetch_valid,
    output wire  [31:0] debug_fetch_vaddr,
    output wire  [31:0] debug_fetch_paddr,
    output wire  [31:0] debug_crmd,
    output wire  [31:0] debug_badv,
    output wire  [31:0] debug_dmw0,
    output wire  [31:0] debug_dmw1,
    output wire         debug_exception_valid,
    output wire  [ 5:0] debug_exception_cause,
    output wire  [31:0] debug_exception_pc,
    output wire  [31:0] debug_exception_inst,
    output wire         debug_ertn,
    output wire  [31:0] debug_gpr4,
    output wire  [31:0] debug_gpr5,
    output wire  [31:0] debug_gpr6,
    output wire  [31:0] debug_gpr7,
    output wire  [31:0] debug_gpr8,
    output wire  [31:0] debug_eentry,
    output wire  [31:0] debug_tlbrentry
`ifdef CPU_2CMT
    ,
...
`endif
    ...
    wire [31:0] irom_vaddr;
    wire        irom_req_cacheable;
    wire        irom_req_kill;
    wire        irom_resp_valid;
    wire [63:0] irom_resp_data;
    wire [13:0] irom_resp_predecode;
    wire        icache_maint_valid;
    wire        dcache_maint_valid;
    wire [31:0] cache_maint_addr;
    wire [ 1:0] cache_maint_mode;
    wire        icache_maint_done;
    wire        dcache_maint_done;
    ...
    wire [31:0] debug0_wb_mem_paddr_i;
    ...
    wire [31:0] debug1_wb_mem_paddr_i;
    ...
    wire [ 4:0] debug_tlbfill_index_i;
    wire [31:0] debug_fetch_vaddr_i;
    ...
    cpu_top #(
        ...
        .TLB_ENTRIES         (TLBNUM),
        .EXTERNAL_CACHE_MAINT(1'b1),
        // Separate I/D caches are present. Geometry encoding follows the
        // OpenLA500 CPUCFG contract: line-log2, set-log2, ways-minus-one.
        .CPUCFG_CACHE_PRESENT(32'h0000_0005),
        .CPUCFG_ICACHE       (32'h0409_0000),
        .CPUCFG_DCACHE       (32'h0509_0001),
        // SoC IRQ[4]=DMA and IRQ[3]=NAND may be shorter than one CPU
        // clock. Other external sources retain architectural level semantics.
        .IRQ_EDGE_MASK       (8'h18)
    ) u_cpu (
        .clk                 (aclk),
        .rst_n               (core_rst_n),
        .irom_addr           (irom_vaddr),
        .irom_req_valid      (irom_req_valid),
        .irom_req_addr       (irom_req_addr),
        .irom_req_cacheable  (irom_req_cacheable),
        ...
        .debug0_wb_mem_paddr (debug0_wb_mem_paddr_i),
        ...
        .debug1_wb_mem_paddr (debug1_wb_mem_paddr_i),
        ...
        .debug_tlbfill_index (debug_tlbfill_index_i),
        .debug_fetch_vaddr   (debug_fetch_vaddr_i)
    );
    ...
    nscscc_axi_bridge u_nscscc_axi_bridge (
        ...
        .irom_resp_predecode(irom_resp_predecode),
        .icache_maint_valid(icache_maint_valid),
        .icache_maint_addr(cache_maint_addr),
        .icache_maint_mode(cache_maint_mode),
        .icache_maint_done(icache_maint_done),
    ...
    assign debug0_wb_mem_read = debug0_wb_mem_read_i;
    assign debug0_wb_mem_write = debug0_wb_mem_write_i;
    assign debug0_wb_mem_addr = debug0_wb_mem_addr_i;
    assign debug0_wb_store_data = debug0_wb_store_data_i;
    assign debug1_wb_mem_read = debug1_wb_mem_read_i;
    assign debug1_wb_mem_write = debug1_wb_mem_write_i;
    assign debug1_wb_mem_addr = debug1_wb_mem_addr_i;
    assign debug1_wb_store_data = debug1_wb_store_data_i;
    assign debug1_wb_rf_wen_raw = debug1_wb_rf_wen_i;
    assign debug1_wb_rf_wnum_raw = debug1_wb_rf_wnum_i;
    assign debug1_wb_rf_wdata_raw = debug1_wb_rf_wdata_i;
...
`ifdef DIFFTEST_EN
    ...
    reg  [ 4:0] cmt0_tlbfill_index;
    reg  [ 7:0] cmt0_ld_valid;
    reg  [ 7:0] cmt0_st_valid;
    reg  [31:0] cmt0_mem_vaddr;
    reg  [31:0] cmt0_mem_paddr;
    ...
    reg  [31:0] cmt1_mem_vaddr;
    reg  [31:0] cmt1_mem_paddr;
    ...
    wire [63:0] cmt0_timer_value = cmt0_cnt_high
                                 ? {cmt0_wdata, 32'd0}
                                 : {32'd0, cmt0_wdata};
    wire cmt0_is_tlbfill = cmt0_inst == 32'h0648_3400;
    ...
    always @(posedge aclk) begin
        if (!core_rst_n) begin
            ...
            cmt0_csr_rstat <= 1'b0;
            cmt0_csr_data <= 32'd0;
            cmt0_tlbfill_index <= 5'd0;
            cmt0_ld_valid <= 8'd0;
            cmt0_st_valid <= 8'd0;
            cmt0_mem_vaddr <= 32'd0;
            cmt0_mem_paddr <= 32'd0;
            cmt0_st_data <= 32'd0;
            cmt1_valid <= 1'b0;
            cmt1_pc <= 32'd0;
            cmt1_inst <= 32'd0;
            ...
            cmt1_ld_valid <= 8'd0;
            cmt1_st_valid <= 8'd0;
            cmt1_mem_vaddr <= 32'd0;
            cmt1_mem_paddr <= 32'd0;
            cmt1_st_data <= 32'd0;
            cmt_excp_valid <= 1'b0;
            cmt_ertn <= 1'b0;
            cmt_intr_no <= 32'd0;
            ...
            cmt0_tlbfill_index <= debug_tlbfill_index_i;
            cmt0_ld_valid <= diff_ld_mask0;
            cmt0_st_valid <= diff_st_mask0;
            cmt0_mem_vaddr <= debug0_wb_mem_addr_i;
            cmt0_mem_paddr <= debug0_wb_mem_paddr_i;
            ...
            cmt1_mem_vaddr <= debug1_wb_mem_addr_i;
            cmt1_mem_paddr <= debug1_wb_mem_paddr_i;
    ...
    DifftestInstrCommit u_difftest_commit0 (
        ...
        .TLBFILL_index(cmt0_tlbfill_index), .is_CNTinst(cmt0_is_cnt),
    ...
    DifftestStoreEvent u_difftest_store0 (
        ...
        .valid(cmt0_st_valid), .storePAddr(diff_zext32(cmt0_mem_paddr)),
        .storeVAddr(diff_zext32(cmt0_mem_vaddr)),
        .storeData(diff_zext32(cmt0_st_data))
    );
    DifftestStoreEvent u_difftest_store1 (
        .clock(aclk), .coreid(8'd0), .index(8'd1),
        .valid(cmt1_st_valid), .storePAddr(diff_zext32(cmt1_mem_paddr)),
        .storeVAddr(diff_zext32(cmt1_mem_vaddr)),
        .storeData(diff_zext32(cmt1_st_data))
    );
    DifftestLoadEvent u_difftest_load0 (
        .clock(aclk), .coreid(8'd0), .index(8'd0),
        .valid(cmt0_ld_valid), .paddr(diff_zext32(cmt0_mem_paddr)),
        .vaddr(diff_zext32(cmt0_mem_vaddr))
    );
    DifftestLoadEvent u_difftest_load1 (
        .clock(aclk), .coreid(8'd0), .index(8'd1),
        .valid(cmt1_ld_valid), .paddr(diff_zext32(cmt1_mem_paddr)),
        .vaddr(diff_zext32(cmt1_mem_vaddr))
    );
...
`endif
    ...
    // The register-file debug read is continuously selected by reg_num, so
    // infor_flag is only a qualifier in the external debug controller.
    assign debug_fetch_valid = irom_req_valid;
    assign debug_fetch_vaddr = debug_fetch_vaddr_i;
    assign debug_fetch_paddr = irom_req_addr;
    assign debug_crmd = debug_priv_state_i[0*32 +: 32];
    assign debug_badv = debug_priv_state_i[6*32 +: 32];
    assign debug_dmw0 = debug_priv_state_i[25*32 +: 32];
    assign debug_dmw1 = debug_priv_state_i[26*32 +: 32];
    assign debug_exception_valid = debug_excp_valid_i;
    assign debug_exception_cause = debug_cause_i;
    assign debug_exception_pc = debug_exception_pc_i;
    assign debug_exception_inst = debug_exception_inst_i;
    assign debug_ertn = debug_ertn_i;
    assign debug_gpr4 = debug_gpr_state_i[4*32 +: 32];
    assign debug_gpr5 = debug_gpr_state_i[5*32 +: 32];
    assign debug_gpr6 = debug_gpr_state_i[6*32 +: 32];
    assign debug_gpr7 = debug_gpr_state_i[7*32 +: 32];
    assign debug_gpr8 = debug_gpr_state_i[8*32 +: 32];
    assign debug_eentry = debug_priv_state_i[7*32 +: 32];
    assign debug_tlbrentry = debug_priv_state_i[24*32 +: 32];

    wire unused_debug_inputs = infor_flag ^ debug1_wb_valid_i
