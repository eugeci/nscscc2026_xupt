// SPDX-License-Identifier: GPL-2.0
/*
 * Non-interactive VisionArm demonstration PID 1 for LA32R Linux.
 *
 * It presents a shell-like transcript while directly performing the tested
 * LCD/camera MMIO operations.  No console input is consumed, so the demo is
 * independent of the UART RX issue under investigation.
 */

#include <dirent.h>
#include <errno.h>
#include <fcntl.h>
#include <stdint.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <sys/mman.h>
#include <sys/mount.h>
#include <sys/stat.h>
#include <sys/types.h>
#include <unistd.h>

#define VISIONARM_PAGE_BASE 0x1fd0e000UL
#define VISIONARM_PAGE_SIZE 0x1000UL

#define CAM_CTRL_OFFSET     0x100U
#define CAM_STATUS_OFFSET   0x104U
#define CAM_ACTIVITY_OFFSET 0x108U
#define CAM_FRAME0_OFFSET   0x114U
#define CAM_FRAME1_OFFSET   0x118U
#define CAM_OUTPUT_OFFSET   0x11cU
#define CAM_INPUT_OFFSET    0x120U
#define CAM_STRIDE_OFFSET   0x124U
#define CAM_MAGIC_OFFSET    0x13cU

#define LCD_CTRL_OFFSET     0x140U
#define LCD_FB_OFFSET       0x144U
#define LCD_STATUS_OFFSET   0x148U
#define LCD_SIZE_OFFSET     0x14cU
#define LCD_MAGIC_OFFSET    0x150U

#define CAM_MAGIC_EXPECTED 0x43414d31U
#define LCD_MAGIC_EXPECTED 0x4c434431U
#define MAX_ROOT_ENTRIES   32U
#define MAX_ROOT_NAME      64U

static volatile uint32_t *visionarm_reg(void *page, unsigned int offset)
{
	return (volatile uint32_t *)((unsigned char *)page + offset);
}

static uint32_t read_reg(void *page, unsigned int offset)
{
	uint32_t value = *visionarm_reg(page, offset);

	__sync_synchronize();
	return value;
}

static void write_reg(void *page, unsigned int offset, uint32_t value)
{
	*visionarm_reg(page, offset) = value;
	__sync_synchronize();
	(void)*visionarm_reg(page, offset);
}

static void attach_console(void)
{
	int fd = open("/dev/console", O_RDWR | O_NOCTTY);

	if (fd < 0)
		return;
	(void)dup2(fd, STDIN_FILENO);
	(void)dup2(fd, STDOUT_FILENO);
	(void)dup2(fd, STDERR_FILENO);
	if (fd > STDERR_FILENO)
		(void)close(fd);
}

static void mount_pseudo_filesystems(void)
{
	(void)mount("proc", "/proc", "proc", 0, NULL);
	(void)mount("sysfs", "/sys", "sysfs", 0, NULL);
}

static int compare_names(const void *left, const void *right)
{
	return strcmp((const char *)left, (const char *)right);
}

static int print_ls(void)
{
	char names[MAX_ROOT_ENTRIES][MAX_ROOT_NAME];
	DIR *directory;
	struct dirent *entry;
	unsigned int count = 0;
	unsigned int index;

	directory = opendir("/");
	if (directory == NULL) {
		printf("ls: cannot open '/': %s\n", strerror(errno));
		return -1;
	}

	while ((entry = readdir(directory)) != NULL &&
	       count < MAX_ROOT_ENTRIES) {
		if (strcmp(entry->d_name, ".") == 0 ||
		    strcmp(entry->d_name, "..") == 0)
			continue;
		(void)strncpy(names[count], entry->d_name,
			      sizeof(names[count]) - 1U);
		names[count][sizeof(names[count]) - 1U] = '\0';
		count++;
	}
	(void)closedir(directory);
	qsort(names, count, sizeof(names[0]), compare_names);

	for (index = 0; index < count; index++)
		printf("%s%s", index == 0 ? "" : "  ", names[index]);
	putchar('\n');
	return 0;
}

static int run_peripheral_demo(void)
{
	int memory_fd;
	void *page;
	uint32_t cam_magic;
	uint32_t cam_status;
	uint32_t activity_before;
	uint32_t activity_after;
	uint32_t lcd_magic;
	uint32_t lcd_status;
	int passed = 1;

	memory_fd = open("/dev/mem", O_RDWR | O_SYNC);
	if (memory_fd < 0) {
		printf("lcdctl: cannot open /dev/mem: %s\n", strerror(errno));
		return -1;
	}

	page = mmap(NULL, VISIONARM_PAGE_SIZE, PROT_READ | PROT_WRITE,
		    MAP_SHARED, memory_fd, (off_t)VISIONARM_PAGE_BASE);
	if (page == MAP_FAILED) {
		printf("lcdctl: MMIO map failed: %s\n", strerror(errno));
		(void)close(memory_fd);
		return -1;
	}

	printf("\n/ # lcdctl show\n");
	lcd_magic = read_reg(page, LCD_MAGIC_OFFSET);
	write_reg(page, LCD_CTRL_OFFSET, 2U);
	usleep(250000);
	lcd_status = read_reg(page, LCD_STATUS_OFFSET);
	if (lcd_magic == LCD_MAGIC_EXPECTED) {
		puts("lcdctl: test card displayed");
	} else {
		printf("lcdctl: bad magic 0x%08x\n", lcd_magic);
		passed = 0;
	}

	printf("\n/ # cam on\n");
	activity_before = read_reg(page, CAM_ACTIVITY_OFFSET);
	write_reg(page, CAM_CTRL_OFFSET, 1U);
	sleep(2);
	cam_status = read_reg(page, CAM_STATUS_OFFSET);
	activity_after = read_reg(page, CAM_ACTIVITY_OFFSET);
	if ((cam_status & (1U << 6)) != 0U &&
	    activity_after != activity_before) {
		puts("cam: camera DMA started");
	} else {
		printf("cam: DMA start failed, status=0x%08x activity=0x%08x->0x%08x\n",
		       cam_status, activity_before, activity_after);
		passed = 0;
	}

	printf("\n/ # cam info\n");
	cam_magic = read_reg(page, CAM_MAGIC_OFFSET);
	printf("Version magic  : 0x%08x (expected 0x%08x)\n",
	       cam_magic, CAM_MAGIC_EXPECTED);
	printf("Sensor input   : 0x%08x\n",
	       read_reg(page, CAM_INPUT_OFFSET));
	printf("DDR/VGA output : 0x%08x\n",
	       read_reg(page, CAM_OUTPUT_OFFSET));
	printf("Line stride    : 0x%08x\n",
	       read_reg(page, CAM_STRIDE_OFFSET));
	printf("Frame buffer 0 : 0x%08x\n",
	       read_reg(page, CAM_FRAME0_OFFSET));
	printf("Frame buffer 1 : 0x%08x\n",
	       read_reg(page, CAM_FRAME1_OFFSET));
	printf("Camera status  : 0x%08x\n", cam_status);
	printf("Activity count : 0x%08x -> 0x%08x\n",
	       activity_before, activity_after);
	printf("LCD magic      : 0x%08x (expected 0x%08x)\n",
	       lcd_magic, LCD_MAGIC_EXPECTED);
	printf("LCD status     : 0x%08x\n", lcd_status);
	printf("LCD size       : 0x%08x\n",
	       read_reg(page, LCD_SIZE_OFFSET));
	printf("LCD framebuffer: 0x%08x\n",
	       read_reg(page, LCD_FB_OFFSET));

	if (cam_magic != CAM_MAGIC_EXPECTED)
		passed = 0;
	puts(passed ? "VisionArm demo  : PASS" : "VisionArm demo  : FAIL");

	(void)munmap(page, VISIONARM_PAGE_SIZE);
	(void)close(memory_fd);
	return passed ? 0 : -1;
}

int main(void)
{
	attach_console();
	setvbuf(stdout, NULL, _IONBF, 0);
	setvbuf(stderr, NULL, _IONBF, 0);
	mount_pseudo_filesystems();

	puts("LA32R Linux");
	puts("");
	puts("/ # ls");
	(void)print_ls();
	(void)run_peripheral_demo();
	puts("");
	printf("/ # ");

	/* PID 1 must never exit; keep the final shell-like prompt unchanged. */
	for (;;)
		sleep(3600);
}
