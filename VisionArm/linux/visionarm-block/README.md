...
visionarm-block --color red --model /models/blocks_v1.xnpu --class 0 \
...
Control mode rejects missing, incomplete, unvalidated, wrong-resolution or
singular calibration files. Check one without touching hardware using:

```sh
visionarm-block --calibration calibration.ini --check-calibration
```
