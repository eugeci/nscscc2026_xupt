...
必须下板测量、当前不能由软件推断的项目见
[闭环演示硬件待确认清单](docs/闭环演示硬件待确认清单.md)。

已验证的比特流与 Linux 内核位于 [`release/`](release/) 目录，可直接用于下板演示。

## NPU 集成分支

统一演示版本以本目录的 OpenLA500/VisionArm 工程为基础，在同一 SoC 中加入
`0x1f100000` NPU、DMA 和中断。首次打开工程前运行：
...
原 `release/` 中的 bitstream 和内核只包含 VisionArm，不能用于 NPU 统一演示；
统一版本必须重新综合 FPGA，并使用仓库根目录 `linux/npu/build.sh` 的 `demo`
模式重新生成内核。
