...
uint32_t
sys_vision_read(uint32_t address) {
    return (uint32_t)syscall(SYS_vision_mmio, 0, address, 0);
}

int
sys_vision_write(uint32_t address, uint32_t value) {
    return syscall(SYS_vision_mmio, 1, address, value);
}

int
sys_vision_lcd_test(void) {
    return syscall(SYS_vision_lcd_test, 0);
}
