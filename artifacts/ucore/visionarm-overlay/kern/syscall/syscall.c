...
static int
sys_vision_mmio(uint32_t arg[]) {
    uint32_t operation = arg[0];
    uint32_t address = arg[1];

    if (operation == VISION_MMIO_READ) {
        return (int)visionarm_mmio_read(address);
    }
    if (operation == VISION_MMIO_WRITE) {
        return visionarm_mmio_write(address, arg[2]);
    }
    return -1;
}

static int
sys_vision_lcd_test(uint32_t arg[]) {
    return visionarm_lcd_test_pattern();
}
...
static int (*syscalls[])(uint32_t arg[]) = {
  ...
  [SYS_dup]               sys_dup,
  [SYS_vision_mmio]       sys_vision_mmio,
  [SYS_vision_lcd_test]   sys_vision_lcd_test,
};
