...
trap_dispatch(struct trapframe *tf) {
  ...
  switch(code){
    ...
    case EX_RI:
      if (!trap_in_kernel(tf)) {
        uintptr_t era = tf->tf_era;
        pte_t *ipte = get_pte(current_pgdir, era, 0);
        PRINT_HEX(" InstD[-1]\t: ", *(volatile uint32_t *)(era - 4));
        PRINT_HEX(" InstD[0]\t: ", *(volatile uint32_t *)era);
        if (ipte != NULL && ptep_present(ipte)) {
          uintptr_t pa = PTE_ADDR(*ipte) | PGOFF(era);
          PRINT_HEX(" Code PTE\t: ", *ipte);
          PRINT_HEX(" Code PA\t: ", pa);
          PRINT_HEX(" Cached[-1]\t: ", *(volatile uint32_t *)(pa - 4));
          PRINT_HEX(" Cached[0]\t: ", *(volatile uint32_t *)pa);
          PRINT_HEX(" Uncache[-1]\t: ", *(volatile uint32_t *)UNCACHE_ADDR(pa - 4));
          PRINT_HEX(" Uncache[0]\t: ", *(volatile uint32_t *)UNCACHE_ADDR(pa));
        }
      }
      print_trapframe(tf);
      if(trap_in_kernel(tf)) {
